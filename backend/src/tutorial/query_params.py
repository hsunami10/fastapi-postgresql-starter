...
async def read_item(offset: int = 0, limit: int = 10) -> list[dict[str, str]]:
...
@router.get("/required/")
async def read_user_item(required_q_param: str) -> dict[str, Any]:
...
@router.get("/ellipses/")
async def read_items(q: Annotated[str, Query(min_length=3)]) -> dict[str, Any]:
