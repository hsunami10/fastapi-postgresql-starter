...
async def read_items2() -> list[dict[str, float | str]]:
    return [
...
# NOTE: mypy type-checking doesn't work for this
...
def fake_save_user(user_in: UserIn3) -> UserInDB:
...
@router.post("/in_out_db_models/", response_model=UserOut3)
async def create_user3(user_in: UserIn3) -> Any:
...
@router.get("/union_types/{item_id}", response_model=PlaneItem | CarItem)
async def read_item(item_id: str) -> dict[str, str | int] | None:
